...
### 랜딩페이지 개편 · 게시 상태 표시 · 이미지 최적화 (2026-08-21, 7차)
...
- [ ] **이 문서의 파일명이 깨져 있다** — 디스크상 실제 이름이 `#Uc9c4#Ud589#Uc0ac#Ud56d#Uc815#Ub9ac.md`
  (`진행사항정리`의 유니코드 코드포인트가 이스케이프된 채 그대로 파일명이 됨). 표시 문제가 아니라
  git에도 그 이름으로 들어가 있다. `git mv` 로 `진행사항정리.md` 로 고치면 되지만, 히스토리 추적이
  끊기지 않는지 확인하고 진행할 것

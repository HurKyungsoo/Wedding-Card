...
### 엔딩(마지막 인사) 사진+문구 섹션 되살림 (2026-08-24)
데어무드가 자기네 1순위 추천 기능으로 꼽는 "엔딩 사진, 문구"를 참고해 8/24 오전에
한 세션이 추가했다가(`d800a4d`) 30분 만에 이유 기록 없이 되돌려졌다(`3cbccf1`). 사용자가
"엔딩사진부터 되살려보자"고 해서 원인부터 찾았다.

**원인**: 하객 화면의 `#endingPhotoImg`(invitation.html)와 편집기 썸네일의
`#endingThumbImg`(admin/edit.html) 둘 다 `th:if`로 렌더돼 있었다. 사진이 없는 청첩장에서는
`<img>`가 DOM에 아예 없으니, 편집기에서 사진을 처음 올려도 `getElementById`가 `null`을
돌려받아 조용히 아무 일도 안 일어났다 — 업로드 영역은 사라지고 썸네일 박스는 나타나는데
그 안에 사진이 없는 상태. 라이브 미리보기도 마찬가지로 반영 안 됨. 이 프로젝트에서 반복적으로
나온 **`th:if` vs `th:style`** 버그 유형과 같다(섹션을 감싸는 div들은 이미 `th:style`로 올바르게
돼 있었는데, 그 안의 `<img>` 태그 두 곳만 놓쳤다).

**고침**: 두 `<img>` 모두 항상 DOM에 두고 `src`를 `th:src` 삼항식으로 조건부 채우도록 변경.
`SECTION_REVERT_HOOKS`에 `ending` 항목도 추가(전엔 없었음 — 나중에 이 섹션에 되돌리기
버튼이 붙으면 업로드 영역/썸네일 표시가 같이 안 맞을 뻔했다).

원래 커밋의 자체 기록에 "admin/edit는 카카오 OAuth 뒤에 있어 실제 브라우저로 못 돌려봤다"고
적혀 있었는데, 그게 재현 안 된 이유였을 가능성이 높다. 이번엔 로컬 덤프로 편집기를 띄워
`handleEndingPhotoFile()` 경로를 실제로 태워 편집기 썸네일·라이브 미리보기·문구 실시간 반영까지
전부 확인했다. `EndingSectionLivePreviewTest` 5개로 회귀 방지 고정.

### 🔴 프로덕션 DB 손상 — 전체 데이터 유실, 빈 DB로 재기동 (2026-08-24)
`main`에 폰트 선택기 커밋을 푸시했더니 빌드는 성공했는데 배포의 헬스체크가 실패했다.
원인을 보니 **내 배포와 무관하게 그날 새벽 3시 12분경부터 이미 H2 파일이 손상돼 있었고**,
서비스가 72회 이상 재시작을 반복하며 계속 다운 상태였다(내 배포가 문제를 만든 게 아니라,
우연히 그 상태에서 재시작을 걸어 처음으로 드러낸 것).

**진단**: `Chunk 2359 not found` — MVStore의 모든 테이블이 공유하는 트랜잭션/타입 레지스트리
페이지 자체가 사라져 있었다. 시도한 것:
- H2 2.4.240(최신)으로 `MVStoreTool -repair`, `-dump`, `Recover` 전부 시도 — repair는
  "Success"를 출력하지만 실제로는 더 깊은 층위의 손상이라 못 고침. Recover는 청크 레이아웃까지는
  읽지만 실제 행 데이터를 디코드하는 단계에서 동일 지점에 걸림
- 03:12에 남아있던 백업본도 **완전히 동일한 손상** — 즉 그보다도 더 이전부터 있던 문제
- 트랜잭션 계층을 우회해 원시 MVStore로 열면 테이블 8개 존재 자체는 확인되지만, 실제 행을
  꺼내려면 H2 내부 직렬화 포맷을 손으로 재구현해야 해서 성공을 보장 못 함
- 이 기기·서버 어디에도 더 오래된 백업이나 자동 백업 체계가 없었음(cron엔 메타스페이스
  로그 스크립트 하나뿐)

사용자 확인 후 **빈 DB로 새로 시작**하기로 결정. 손상된 파일들은 지우지 않고
`/home/ubuntu/incident-2026-08-24-db-corruption/`에 전부 보관해뒀다(나중에 더 깊은 복구를
시도하고 싶어질 경우를 대비). 서비스 재기동 후 `NRestarts=0`으로 안정, 헬스체크 정상.

**결과: 그때까지 저장돼 있던 청첩장·계정·방명록·조회 로그 데이터가 전부 유실됐다.**
방문자 통계·조회수도 초기화됨.

⚠️ **재발 방지가 최우선이다** — 지금도 자동 백업이 없다. 아래 "최우선"에 추가함.
...
- [x] **DB 자동 백업 체계 구축** (2026-08-24 완료) — `/home/ubuntu/backup-db.sh`가 `wedding-db.mv.db`를 gzip으로 압축해 `/home/ubuntu/db-backups/`에 저장, 14일 지난 것은 자동 삭제. crontab에 `10 */6 * * *`로 등록(6시간마다: 00:10/06:10/12:10/18:10). **앱을 멈추거나 손대지 않는 원시 파일 복사 방식** — MVStore는 마지막 커밋 청크까지는 항상 유효해서, 실행 중인 파일을 그냥 복사해도 쓸 수 있는 백업이 된다. 실제로 압축 해제 후 JDBC로 열어 테이블 조회까지 되는 것 확인함
  - ⚠️ 한계: 이 백업도 결국 같은 `.mv.db` 포맷이라, 8/24처럼 파일 내부 깊은 곳(트랜잭션/타입 레지스트리)이 손상되는 유형의 문제가 그 순간에 발생하면 그 회차 백업도 못 쓸 수 있다. 더 안전하게 하려면 `AUTO_SERVER=TRUE`로 datasource URL을 바꿔서 H2의 `BACKUP TO`(SQL) 또는 `Script` 도구로 **논리적 백업(SQL 텍스트)**을 따로 뜨는 걸 고려할 것 — 다만 이건 프로덕션 datasource 설정을 건드리는 일이라 별도로 확인받고 진행하는 게 맞음
  - 백업 위치: `/home/ubuntu/db-backups/wedding-db-YYYYMMDD-HHMMSS.mv.db.gz`, 로그: 같은 디렉터리의 `backup.log`
  - 복원: `gunzip -c <파일> > data/wedding-db.mv.db` 후 서비스 재시작
